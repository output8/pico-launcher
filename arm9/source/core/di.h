...
public:
    NoDestruct()
        : _pointer(nullptr) { (void)sizeof(T); }

    explicit NoDestruct(T* pointer)
        : _pointer(pointer) { (void)sizeof(T); }
  ...
  class scope_impl<T, aux::true_type> {
   ...
   private:
    ...
    auto create_impl(const TProvider& provider) {
      static NoDestruct<T_> object{provider.get()};
      return wrappers::shared<singleton, T_, NoDestruct<T_>&>{object};
    }
  };
